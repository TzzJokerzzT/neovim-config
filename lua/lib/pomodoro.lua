...
start_standby = function()
  ...
  notify(
    string.format(
      "⏸️ ¡Tiempo de espera! Prepárate para el descanso... (%s)",
...
function M.setup()
  ...
  vim.keymap.set("n", "<leader>ps", function()
    M.start_interactive()
  end, { desc = "Start Pomodoro session (interactive: work → break → standby)" })
  ...
  vim.keymap.set("n", "<leader>pw", function()
    M.set_work_interactive()
  end, { desc = "Start Pomodoro (interactive: work → break → standby)" })
  vim.keymap.set("n", "<leader>pb", function()
    M.set_break_interactive()
  end, { desc = "Start Pomodoro (interactive: work → break → standby)" })
  vim.keymap.set("n", "<leader>pc", function()
    M.configure_interactive()
  end, { desc = "Configure default Pomodoro times (interactive: work → break → standby)" })

  -- Additional quick start keymaps
  vim.keymap.set("n", "<leader>p1", function()
    M.start_session("25min", "5min")
  end, { desc = "Quick start: 25min work, 5min break" })
  vim.keymap.set("n", "<leader>p2", function()
    M.start_session("45min", "10min")
  end, { desc = "Quick start: 45min work, 10min break" })
  vim.keymap.set("n", "<leader>p3", function()
    M.start_session("90min", "20min")
  end, { desc = "Quick start: 90min work, 20min break" })
end
